...
// Used to smooth out Freddy's jaw's movements
double lerp(double a, double b, double t)
{
  double res = a + ((b - a) * t);
  if (isnan(b)) {b = 5;} // Fixes the fact that sometimes target is nan
  if (isnan(res)) {res = b;} // If one number is nan, res is nan :(
  return res;
}

// Sets up everything basically
void setup() {
  Serial.begin(9600);
...
void loop() {
  ...
  // Reads incoming chunks if speaker is ready to play audio
  if(speaker.ready())
  {
    readChunk();
  }
  ...
  {
    // Rotate the jaw according to sound volume (if there's sound at all)
  ...
  else  // Attack mode functionality - overrides everything until done
  {
    ...
    // Attack mode eye control
    strip.setPixelColor(0, (FreddyLightOn ? FreddyAttackLight : 0x000000));
    // When on music box state, flicker eyes
...
// Almost everything below was taken from the example folder of the speaker library. It involves memory manipulation and wav file reading, so for a 1000-level *EE* course I think it's okay not to handwrite this part manually <3 not that i would know how to do that o_o;;
...
int selectFile(const char* filename) {
  ...
  if (wavFile) {
    memset((uint8_t*)data, 0x80, sizeof(data)); // resets buffer to bias value 0x8080 (quicker via memset() than writing 0x8000 in a loop) (whatever THAT means!)
...
int readChunk() {
  ...
  if (wavFile.isOpen()) {
    ...
    for(int b = 0; b < n; b += wh.sample_alignment) {
      ...
      meanSquareValue += ((int16_t)(wav[retVal-1]))*((int16_t)(wav[retVal-1])); // add squares to mean square value
    }
    

    meanSquareValue = sqrt(meanSquareValue); // spits out the average volume
    soundVolume = sqrt(1048525 - meanSquareValue); // post processing - makes movement more sensitive to lower volumes, and less to higher, given sqrt is an increasing+concave down function B)
  }
  else
  {
    speaker.end();
